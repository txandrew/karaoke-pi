...
#os.system("ip addr | mail -s \"Karaoke-Pi Setup\" anderson.andrew.r@gmail.com")
...
def setNextSong():
    ...
    if ( int(nxt_cur.rowcount) > 0 ):
        ...
        nxt_cur.execute("UPDATE tbl_status SET status='PLAYING', youtube_id='" + nxt_ytid + "', queued_by='" + nxt_qued + "';")
...
def resetVideo():
    ...
    os.system("/var/www/html/karaoke/server/killKaraoke.sh")
...
while bool_run:
    ...
    if str_n_status == "RESET_VID":
        str_function += "[RESET VIDEO : " + resetVideo() + "]"

    if str_n_status == "PLAYING" and str_n_youtube == "-1":
    ...
    if (str_c_status == "STARTING" and str_n_status == "STANDBY" ):
        str_function += setWaiting()
